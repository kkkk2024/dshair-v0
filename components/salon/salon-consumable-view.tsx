...
export function SalonConsumableView({ product, locale = 'en' }: { product: Consumable; locale?: Locale }) {
  const c = getSalonSuppliesContent(locale)
  const t = getShopUi(locale)
  const basePath = localeHref('/salon-supplies', locale)

  const whatsappLink = `https://wa.me/8613516946001?text=${encodeURIComponent(c.product.whatsappMsg(product.name))}`
        ...
        <main className="flex-1">
          ...
          <div className="border-b">
            <div className="container px-4 md:px-6 py-3 max-w-6xl mx-auto">
              <nav className="flex items-center gap-2 text-sm text-muted-foreground">
                <Link href={localeHref('/', locale)} className="hover:text-foreground">{t.productDetail.breadcrumbHome}</Link>
                <span>/</span>
                <Link href={basePath} className="hover:text-foreground">{c.product.breadcrumbSupplies}</Link>
                <span>/</span>
          ...
          <section className="py-8 md:py-16">
            <div className="container px-4 md:px-6 max-w-6xl mx-auto">
              <Link href={basePath} className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground mb-8 transition-colors">
                <ArrowLeft className="h-4 w-4" /> {c.product.backToSupplies}
              </Link>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
                ...
                <div className="space-y-6">
                    ...
                    {isFree ? (
                      <div className="space-y-2">
                        <div className="flex items-baseline gap-3">
                          <span className="text-4xl font-bold text-green-600">{c.cardFree}</span>
                          <span className="text-muted-foreground text-sm">{c.product.postageNote}</span>
                        </div>
                        <p className="text-sm text-muted-foreground">{c.product.onePerSalon}</p>
                      </div>
                    ) : (
                      <div className="space-y-2">
                        <div className="flex items-baseline gap-3">
                          ...
                          {product.originalPrice && (
                            <>
                              ...
                              <span className="text-sm font-semibold text-green-600 bg-green-50 px-2 py-0.5 rounded-full">{c.product.savePercent(savingsPercent)}</span>
                            </>
                          )}
                        </div>
                        <p className="text-xs text-muted-foreground">{c.product.freeShippingNote}</p>
                      </div>
                    )}
                  </div>
                  ...
                  {/* Features */}
                  <div>
                    <h3 className="font-semibold mb-3">{c.product.keyFeatures}</h3>
                  ...
                  {product.whatIncluded && product.whatIncluded.length > 0 && (
                    <div className="bg-muted/30 rounded-2xl p-5">
                      <h3 className="font-semibold flex items-center gap-2 mb-3">
                        <Package className="h-4 w-4 text-primary" /> {c.product.whatIncluded}
                      </h3>
                  ...
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    {product.weight && (
                      <div className="bg-muted/30 rounded-xl p-3">
                        <p className="text-xs text-muted-foreground">{c.product.weight}</p>
                        <p className="font-medium">{product.weight}</p>
                      </div>
                    )}
                    {product.monthlyUsage && (
                      <div className="bg-muted/30 rounded-xl p-3">
                        <p className="text-xs text-muted-foreground">{c.product.monthlyUsage}</p>
                        <p className="font-medium">{product.monthlyUsage}</p>
                      </div>
                    )}
                    {product.profitMargin && (
                      <div className="bg-muted/30 rounded-xl p-3">
                        <p className="text-xs text-muted-foreground">{c.product.margin}</p>
                        <p className="font-medium">{product.profitMargin}</p>
                      </div>
                    )}
                    <div className="bg-muted/30 rounded-xl p-3">
                      <p className="text-xs text-muted-foreground">{c.product.forLabel}</p>
                    ...
                    {product.stripeLink && product.stripeLink.includes("YOUR_LINK") ? (
                      <a href={whatsappLink} target="_blank" rel="noopener noreferrer" className="flex-1">
                        <Button className="w-full rounded-full bg-green-600 hover:bg-green-700 text-white" size="lg">
                          <ShoppingBag className="mr-2 h-5 w-5" /> {t.productDetail.orderWhatsApp}
                        </Button>
                      </a>
                    ) : (
                      <a href={product.stripeLink} target="_blank" rel="noopener noreferrer" className="flex-1">
                        <Button className="w-full rounded-full" size="lg">
                          {c.product.buyNow(product.price)}
                        </Button>
                      </a>
                    )}
                  ...
                  <div className="flex items-center gap-4 text-xs text-muted-foreground pt-2">
                    <span className="flex items-center gap-1"><Truck className="h-3.5 w-3.5" /> {c.product.freeUkDelivery}</span>
                    <span className="flex items-center gap-1"><Clock className="h-3.5 w-3.5" /> {c.product.dispatchesNextDay}</span>
                  </div>
                </div>
              </div>
              ...
              {product.usageInstructions && (
                <div className="mt-12 p-6 bg-muted/20 rounded-2xl border max-w-3xl">
                  <h3 className="font-semibold mb-2">{t.productDetail.tabs.howToUse}</h3>
          ...
          <section className="py-12 bg-muted/30">
            <div className="container px-4 md:px-6 max-w-3xl mx-auto text-center space-y-4">
              <h3 className="font-serif text-xl font-medium">{c.product.tryBeforeTitle}</h3>
              <p className="text-sm text-muted-foreground">
                {c.product.tryBeforeText}
              </p>
              <Link href={`${basePath}/free-sample-pack`}>
                <Button variant="outline" className="rounded-full">
                  <Gift className="mr-2 h-4 w-4" /> {c.product.getFreeSample} <ArrowRight className="ml-2 h-4 w-4" />
                </Button>
              </Link>
            </div>
          </section>
