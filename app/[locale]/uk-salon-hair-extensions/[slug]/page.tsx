...
import { getLocalizedSeoPage, getLocalizedSeoPages } from "@/lib/i18n/seo-pages"
...
export function generateStaticParams() {
  ...
  return locales.flatMap((locale) =>
    seoPages.map((p) => ({ locale, slug: p.slug })),
  )
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const url = `${SITE_URL}/${locale}/uk-salon-hair-extensions/${page.slug}`
  ...
  return {
    ...
    alternates: {
      canonical: url,
      languages: hreflangAlternates(`/uk-salon-hair-extensions/${page.slug}`),
    },
    openGraph: {
      title: page.metaTitle,
      description: page.metaDescription,
...
export default async function LocaleSalonSeoPage({ params }: PageProps) {
  const { locale, slug } = await params
  if (!isLocale(locale) || locale === defaultLocale) notFound()
  const page = getLocalizedSeoPage(slug, locale)
  const localeLocale = locale as Locale

  if (!page) {
    notFound()
  }

  const pageUrl = `${SITE_URL}/${locale}/uk-salon-hair-extensions/${page.slug}`
              ...
              <div className="max-w-4xl">
                ...
                <div className="mt-8 flex flex-wrap gap-3">
                  <Button className="bg-amber-500 hover:bg-amber-600 text-white" asChild>
                    <Link href={localeHref("/salon-partners", localeLocale)}>
                      {page.cta}
                      <ArrowRight className="h-4 w-4" />
                    </Link>
                  </Button>
                  <Button variant="outline" className="border-white/30 text-white hover:bg-white/10" asChild>
                    <Link href={localeHref("/contact", localeLocale)}>Talk to the Team</Link>
                  </Button>
                </div>
              </div>
            </div>
          ...
          <section className="border-b bg-[#FDF8F0]">
            <div className="container px-4 md:px-6 py-3">
              <nav className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
                <Link href={localeHref("/", localeLocale)} className="hover:text-[#4A1942]">Home</Link>
                <span>/</span>
                <Link href={localeHref("/trade-wholesale", localeLocale)} className="hover:text-[#4A1942]">Trade Wholesale</Link>
                <span>/</span>
            ...
            <div className="grid gap-10 lg:grid-cols-[1fr_320px]">
              ...
              <aside className="lg:sticky lg:top-24 h-fit rounded-2xl border bg-white p-6 shadow-sm">
                ...
                <Button className="mt-5 w-full bg-[#4A1942] hover:bg-[#35122f]" asChild>
                  <Link href={localeHref("/salon-partners", localeLocale)}>{page.cta}</Link>
                </Button>
                <div className="mt-6 border-t pt-5">
                  <h3 className="text-sm font-semibold text-[#4A1942]">Useful links</h3>
                  <div className="mt-3 space-y-2">
                    {page.internalLinks.map((link) => (
                      <Link key={link.href} href={localeHref(link.href, localeLocale)} className="block text-sm text-muted-foreground hover:text-[#4A1942]">
                        {link.label}
                      </Link>
                    ))}
                  </div>
