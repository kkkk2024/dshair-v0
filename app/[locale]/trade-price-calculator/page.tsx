...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const c = getTradeCalcContent(locale as Locale)
...
export default async function LocaleTradeCalculatorPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params
  ...
  return (
    <>
      ...
      />
      <TradePriceCalculatorView locale={locale as Locale} />
    </>
  )
}
